...
describe('Test read file with sourceCode', () => {
    it('Read a file', async () => {
        ...
        const expectedTokenIsa: IToken = {lexema: 'isabela', classe: 'ID', tipo: 'NULO'}
        const expectedTokenComp: IToken = {lexema: 'compiladores', classe: 'ID', tipo: 'NULO'}
        ...
        expect(response.token).toEqual(expectedTokenIsa)
        response = await scanner(pathName, response.position)
        expect(response.token).toEqual(expectedTokenComp)
    })
